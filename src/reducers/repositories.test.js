...
vi.mock('../actions/repositories')
commitUserStars.mockImplementation(() => vi.fn())
...
describe('Repositories Reducer', () => {
  ...
  it("should initialize repos' collection", () => {
    const initialState = { repos: [], starred: {} }

    const payload = { items: [{ name: 'test repo 1' }, { name: 'test repo 2' }] }

    const expected = { availableLanguages: {}, repos: [{ name: 'test repo 1' }, { name: 'test repo 2' }], starred: {} }
  ...
  it("should initialize starred repos' hash", () => {
    ...
    const payload = { 'http://example.com': true, 'https://googol.com': false }

    const expected = { repos: [], starred: { 'http://example.com': true, 'https://googol.com': false } }
  ...
  it('should toggle starred state for an existing repo', () => {
    const initialState = { repos: [], starred: { 'http://example.com': true } }

    expect(
      repositories(initialState, { type: TOGGLE_STAR, payload: 'http://example.com' }).starred['http://example.com']
    ).toBeFalsy()
